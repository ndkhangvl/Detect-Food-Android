...
//            float confidences = result.getConfidence();
        ...
        if(maxpos == -1) {
            recyclerView.setVisibility(View.GONE);
            if(SharedData.ngonngu == 1 ) {
                txtResult.setText("Cannot recognize, please try again.");
            } else {
                txtResult.setText("Không thể nhận dạng, vui lòng thử lại.");
            }
        } else {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            if (requestCode == 2) {
                ...
                if(SharedData.ngonngu == 1 ) {
                    loading.setMessage("Please wait, application is in the process of detection");
                } else {
...
//                imgHinh.setImageBitmap(imgChoose);
                ...
                System.out.println("view width : "+imgChoose.getWidth());
                System.out.println("view height: "+imgChoose.getHeight());
                System.out.println("view : "+dimension);
                imgChoose = Bitmap.createScaledBitmap(imgChoose, imgSize, imgSize, false);
//                Matrix matrix = new Matrix();
//                matrix.postRotate(0);
//                Bitmap rotatedBitmap = Bitmap.createBitmap(imgChoose, 0,0,imgSize,imgSize,matrix, false);
                imgHinh.setImageBitmap(imgChoose);
//                imgHinh.setImageBitmap(rotatedBitmap);
